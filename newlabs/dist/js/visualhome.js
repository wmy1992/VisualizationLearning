...
function jsonError(d, textStatus, error) {
    ...
    $("#example_tips").html("注意: key必须是带双引号的字符串<br> 详情参考Json格式<br>");
    $(".error_tips").show();
}
...
function parseError(error, imgstr) {
    ...
    //$("#error_data").html("responsText:<br>" + d.responseText);
    $("#example_tips").html("注意: key必须是带双引号的字符串<br> 详情请参考Json格式<br>"); 
    $(".error_tips").show();
}
...
function initPage() { 
    $(function () { 
            /*$(".error_tips").hide();
    	        ...
    	        /*$('#sidebar-menu>li').live('click',function(){
    	        	console.log($(this).val());
    	        });*/
    	        
    	        //像在main中动态显示，暂时没想出好的方法
    	        /*$('.sidebar-menu li a').click(function(){
    	        	var href=($(this).attr('href'));
    	        //	var fileArray = getFileURL("./example/"+href+".json");
    	        	var ecOption = parseJson("./example/"+href+".json");
    	        });*/
    	        
    	        var opt=option("scatter",15);
    	        require(
	        	        [
	        	            'echarts',
    	        ...
    	        //暂时调用option.js显示
    	        $('.sidebar-menu li a').click(function(){
    	        	var chartType=($(this).attr('href')).substring(1);
    	        	var opt=option(chartType,10);
    	        	console.log(opt);
    	        	require(
    	        	        [
    	        	            'echarts',
    	        	             'echarts/chart/'+ chartType
    	        	        ],
    	        	        function (ec) {
    	        	            var myChart = ec.init(document.getElementById('main'));
    	        	            myChart.setOption(opt,true);
...
function checkJSONIntegrity(option) {
   ...
   "line" == option.type) {
        ...
        //此处添加修改内容
    	var scatterImg = "./img/json_scatter.png";
    	if (isNull(option.series)) {
            parseError("The field series is null", scatterImg);
            return false;
        }
        if (0 == option.series.length) {
            parseError("The length of field series is 0", scatterImg);
            return false;
        }
        if (isNull(option.xAxis)) {
            parseError("The field xAxis is null",scatterImg);
            return false;
        }
        if (0 == option.xAxis.length) {
            parseError("The length of field xAxis is 0",scatterImg);
            return false;
        }
        if (isNull(option.yAxis)) {
            parseError("The field xAxis is null",scatterImg);
            return false;
        }
        if (0 == option.yAxis.length) {
            parseError("The length of field xAxis is 0",scatterImg);
            return false;
        }
...
function parseJson(fileURL) {
    ...
    $.getJSON(fileURL, function (data) {
           ...
           // alert("test parseJson"+data.type+data);
             paintGraph(data);
...
function getGraphName(){
    ...
    return hrefNow.substring(location.href.lastIndexOf("#")+1);
}
...
function paintGraph(option) {
    ...
    /*$("#backhome").click(function(){
    	alert("test Option"+option.type+option.legend);	
    });*/
	...
	"line" == option.type){
       ...
       // console.log(option.title.text);
		...
		console.log("option+chatter"+option);
    } 
